...
export default function ExercisesPage() {
  ...
  async function handleDelete(force: boolean) {
    ...
    try {
      ...
      setDeleteModal(prev =>
        prev
          ? { ...prev, deleting: false, error: err.message || 'Delete failed' }
          : null,
      )
    }
  }
              ...
              deleteModal.usage.total > 0 ? (
                <div className='mb-4 rounded-lg border border-error/30 bg-error/5 p-3 text-sm'>
                  <p className='font-medium text-error mb-2'>
                    Used in {deleteModal.usage.total} schedule item
                    {deleteModal.usage.total !== 1 ? 's' : ''}
                  </p>
                  <div className='flex gap-4 text-xs text-on-surface-variant'>
                    <span>
                      Past:{' '}
                      <span className='font-medium text-on-surface'>
                        {deleteModal.usage.past}
                      </span>
                    </span>
                    <span>
                      Today:{' '}
                      <span className='font-medium text-on-surface'>
                        {deleteModal.usage.current}
                      </span>
                    </span>
                    <span>
                      Future:{' '}
                      <span className='font-medium text-on-surface'>
                        {deleteModal.usage.future}
                      </span>
                    </span>
                  </div>
                  {!isRoot && (
                    <p className='mt-2 text-xs text-on-surface-variant'>
                      Deactivate the exercise instead of deleting it. Force
                      delete is root-only.
                    </p>
                  )}
                  {isRoot && (
                    <p className='mt-2 text-xs text-on-surface-variant'>
                      As root, you can force-delete. Schedule items will have
                      their exercise reference cleared (set to null).
                    </p>
                  )}
                </div>
              ) : (
                <p className='mb-4 text-sm text-on-surface-variant'>
                  This exercise is not used in any schedule. It can be safely
                  deleted.
                </p>
              )
            ) : null}
