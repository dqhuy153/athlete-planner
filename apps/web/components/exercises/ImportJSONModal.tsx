...
function DataField({
  ...
  return (
    <div className='flex items-baseline justify-between gap-3 py-1.5'>
      ...
      <span
        className={`text-caption text-text-primary text-right break-words ${mono ? 'font-data' : ''}`}
      >
        {value}
      </span>
    </div>
...
export function ImportJSONModal({ onClose, onSuccess }: Props) {
    ...
    reader.onload = ev => {
      try {
        ...
        const valid = parsed.filter(
          (item): item is FlatExerciseImportItem =>
        ...
        if (valid.length !== parsed.length) {
          setError(
            t('invalidItemsRemoved', { count: parsed.length - valid.length }),
          )
        }
        setItems(valid)
      } catch {
  ...
  const updateItem = (
    index: number,
    field: keyof FlatExerciseImportItem,
    value: string,
  ) => {
    const next = [...items]
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-4'>
      <div className='w-full max-w-2xl bg-surface-1 rounded-2xl border border-border flex flex-col max-h-[92vh]'>
        <div className='flex items-center justify-between px-5 py-3 border-b border-border shrink-0'>
          <div className='flex items-center gap-2'>
            {step !== 'sport' && (
              <button
                onClick={() => setStep('sport')}
                className='p-2 rounded-lg hover:bg-surface-2 min-h-[44px]'
              >
              ...
              items.length === 0 ? (
                ...
                <div className='space-y-3'>
                  <p className='text-xs text-text-tertiary'>
                    {t('readyToImport', { count: items.length })}
                  </p>
                      ...
                      >
                        <div className='flex-1 grid grid-cols-2 gap-2'>
                          ...
                          <select
                            ...
                            onChange={e =>
                              updateItem(index, config.field, e.target.value)
                            }
                        ...
                        <button
                          ...
                          className='p-1.5 rounded text-text-tertiary hover:text-red-400 min-h-[44px] flex items-center'
                        >
          ...
          <div className='flex-1 flex flex-col min-h-0 p-5 gap-3'>
            ...
            <div className='flex flex-wrap items-center gap-2 px-1 shrink-0'>
              <SummaryCounter
                ...
                count={
                  preview.filter(p => p.status === 'admin-existing').length
                }
              />
              <SummaryCounter
                tone='amber'
                ...
                count={
                  preview.filter(p => p.status === 'custom-existing').length
                }
              />
              <SummaryCounter
                tone='accent'
                label={t('summaryNew')}
                count={preview.filter(p => p.status === 'new').length}
              />
            </div>
              ...
              {preview.map((p, index) => (
                ...
                >
                  ...
                  <select
                    value={p.action}
                    onChange={e =>
                      updateAction(index, e.target.value as ItemAction)
                    }
                  ...
                  >
                    ...
                    {p.status === 'admin-existing' && (
                      <option value='clone'>{t('actionClone')}</option>
                    )}
                    {p.status === 'custom-existing' && (
        ...
        <div className='px-5 py-3 border-t border-border flex gap-3 shrink-0'>
          <button
            onClick={onClose}
            className='flex-1 min-h-[48px] rounded-xl border border-border text-sm text-text-secondary hover:border-accent/40'
          >
            {tc('cancel')}
          </button>
          {step === 'input' && items.length > 0 && (
            <button
              ...
              disabled={loading}
              className='flex-1 min-h-[48px] rounded-xl bg-accent text-black font-semibold text-sm disabled:opacity-40'
            >
...
function DetailSheetBody({
  ...
  const altName = getLocalizedName(
    { name: data.name, vietnameseName: undefined } as {
      name?: string
  ...
  return (
    <div>
      ...
      <div className='sticky top-0 z-10 bg-surface-1 border-b border-border'>
        <div className='flex items-center justify-between px-4 py-2'>
          ...
          <button
            onClick={onClose}
            aria-label='Close'
            className='p-1.5 rounded-lg text-text-secondary hover:text-text-primary hover:bg-surface-2 transition-colors min-h-[36px] min-w-[36px] flex items-center justify-center'
          >
            <X size={18} aria-hidden />
          </button>
        </div>
      ...
      <div className='p-5'>
        ...
        {/* Action selector is rendered as the BottomSheet `footer` prop,
            outside the scrollable area, so it sits flush at the visual bottom
            with no BottomNav-clearance padding below it. */}
      </div>
    </div>
  )
}
