...
- 2026-06-03: localization + visual refactor — the preview popup and detail sheet now mirror the library details page layout (`card-surface` sections, `text-caption font-semibold uppercase tracking-wider` headers, `font-data` indices for instructions, accent-color chips for sport/muscle metadata, `text-subheading font-bold` title block, big `font-data text-accent` index numbers for workout phases). Every field label, enum value (sportType/muscleGroup/runningType/phaseType), phase field, HR zone, and unit (min/km/m/spm/bpm/min-per-km) is fully localized via new `importJSON.{field,enum,phaseField,hrZone,unit}` namespaces in `vi.json` and `en.json`. Summary counters, status badges (`StatusBadge`), and metadata chips (`MetaChip`) replace raw colored text. The detail sheet action selector is now two side-by-side buttons (skip + primary action) with clear selected states. Modal stays at 767 lines (under 800-line budget).
- 2026-06-03: bulk delete — `POST /exercises/private/bulk-delete` and `GET /exercises/private/usage` endpoints; multi-select mode on `/[locale]/library/my` via `ExerciseCard` `selectable` prop; `ScheduleItem.privateExerciseId` FK is `onDelete: SetNull` so deleted exercises leave schedule history intact (payload preserved, name lost). New i18n namespace: `library.bulkDelete`.
